...
export function SearchClient({ q, categoryId, sort, brand, minPrice, maxPrice, semantic, page }: SearchClientProps) {
  const router = useRouter();

  const [priceMin, setPriceMin] = useState(minPrice);
  const [priceMax, setPriceMax] = useState(maxPrice);

  // No auth gate on this query, so unlike other pages that read searchParams (e.g.
  // orders, gated on `enabled: !!user`), it would otherwise resolve during the server
  // render and stream real results while the client's first hydration pass starts from
  // an empty query cache — the resulting markup mismatch leaves the page stuck on a
  // loading state forever on a hard navigation. Deferring the fetch to after mount keeps
  // the server and the client's first paint in the same (loading) state.
